...
def sample_params(n_samples : int) -> np.ndarray:
    LogLambda = _rng.uniform(18.0, 22.0, size = n_samples) #Unification scale.
    g0 = _rng.uniform(0.0, 1.0, size = n_samples) #Initial coupling constant unified at Lambda
    Logve = _rng.uniform(12, 20, size = n_samples)
    Logvmu = _rng.uniform(12, 20, size = n_samples)
    Logvtau = _rng.uniform(12, 20, size = n_samples)
